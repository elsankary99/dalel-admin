...
import 'package:dalel_admin/view/widgets/add_books_widget/add_books_form.dart';
...
@RoutePage()
class AddBazarBooksPage extends ConsumerWidget {
  const AddBazarBooksPage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final provider = ref.read(addHistoryBookProvider.notifier);
    final state = ref.watch(addHistoryBookProvider);
    final imageProvider = ref.read(getImageProvider.notifier);
    ref.listen(addHistoryBookProvider, (previous, next) {
      if (next is AddHistoryBookError) {
        customToast(title: next.message, color: Colors.red);
      }
      if (next is AddHistoryBookSuccess) {
        customToast(
            title: "Periods Added Successfully", color: AppColors.primaryColor);
        ref.invalidate(getHistoryBookProvider);
      ...
      body: Padding(
        ...
        child: CustomScrollView(
          ...
          slivers: [
            ...
            const SliverToBoxAdapter(child: AddBooksForm()),
            SliverToBoxAdapter(child: SizedBox(height: context.height * 0.2)),
            SliverToBoxAdapter(
                child: CustomButton(
              child: state is AddHistoryBookLoading
